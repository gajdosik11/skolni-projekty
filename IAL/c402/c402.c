...
**/
	...
	if (RootPtr != NULL)
	{
		Leftmost_Preorder(RootPtr, &SP);
		while(!SEmptyP(&SP))
		{
			RootPtr = STopPopP(&SP);
			Leftmost_Preorder(RootPtr->RPtr, &SP);
		}
	}
...
**/
	...
	if (RootPtr != NULL)
	{
		Leftmost_Inorder(RootPtr, &SP);
		while(!SEmptyP(&SP))
		{
			RootPtr = STopPopP(&SP);
			BTWorkOut(RootPtr);
			Leftmost_Inorder(RootPtr->RPtr, &SP);
		}
	}
...
**/
	...
	tStackP SP;
	SInitP(&SP);
	while((*RootPtr) != NULL || !SEmptyP(&SP))
	{
		if ((*RootPtr) != NULL)
		{
			tmp = (*RootPtr);
			if ((*RootPtr)->RPtr != NULL)
				SPushP(&SP,(*RootPtr)->RPtr);
